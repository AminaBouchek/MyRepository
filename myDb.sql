...
select count(id_intern), name_cours from intern 
join  relation on intern.id_sp = relation.id_spec 
join cours on cours.id_cours = relation.id_relation  
group by name_cours;

--- Afficher le cour qui est suivi par le plus de stagiaire 
with tmp as(select cs.name_cours, count(*) folow 
from cours cs 
inner join relation rl on rl.id_cours = cs.id_cours 
inner join speciality sp on sp.id_sp = rl.id_spec 
inner join intern ins on ins.id_sp = sp.id_sp 
...
--- better query
select avg(nbr) as my_result 
from (
        SELECT sp.id_sp, count(*)nbr 
        FROM (
              select cr.name_cours, cr.id_cours , count(*) nbr 
              from cours cr 
                   inner join relation rl on rl.id_cours = cr.id_cours 
              group by cr.name_cours, cr.id_cours 
              having count(*) >2)rel 
        inner join relation rl on rl.id_cours = rel.id_cours 
        inner join speciality sp on sp.id_sp = rl.id_spec
        inner join intern nt on nt.id_sp = sp.id_sp 
group by sp.id_sp
        )req;
...
select count(id_cours) , sp.name_sp, sp.id_sp
from speciality sp inner join relation rl on sp.id_sp = rl.id_spec
where id_sp in (
                select id_spec 
                  from (select count(*), id_spec
                        from relation 
                group by id_spec having count(*) > 2)req1) 
and id_sp in (
                select id_sp 
                from (select count(*), id_sp 
                      from intern
                group by id_sp having count(*) >= 6)req2)
group by sp.name_sp, sp.id_sp;
                      



"""with name_query as <<< cte common table expression"""
